       ...
       methods = list(
        ...
        # (binary classification task)
                    fit = function(X, y)
                    {
                         ...
                         print(prob_sentiments)
                         p_neg = prob_sentiments[1,]$n/sum(prob_sentiments$n)
                         p_neu = prob_sentiments[2,]$n/sum(prob_sentiments$n)
                         p_pos = prob_sentiments[3,]$n/sum(prob_sentiments$n)
                         ...
                         fdict$prob_positive <<- seq(0,0, length=length(fdict$word))
                         fdict$prob_neutral <<- seq(0,0, length=length(fdict$word))
                         fdict$prob_negative <<- seq(0,0, length=length(fdict$word))
                         for(i in 1:20){
                           fdict[i,]$prob_positive <<- (positive[positive$token==fdict[i,"word"],]$n)/ fdict[i,]$total
                           fdict[i,]$prob_neutral <<- (neutral[neutral$token==fdict[i,"word"],]$n)/ fdict[i,]$total
                           fdict[i,]$prob_negative <<- (negative[negative$token==fdict[i,"word"],]$n)/ fdict[i,]$total
                           #print(positive[positive$token==fdict[i,"word"],])
